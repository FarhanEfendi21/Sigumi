...
#       },
#     },
# but we just want to replace the intent block end.
for intent, sas in sasak_responses.items():
    ...
    # },
    # },
    # Just look for `'intent': {` and the corresponding `    },` that closes the intent block.

    intent_start_str = f"'{intent}': {{"
    intent_start_idx = content.find(intent_start_str)
